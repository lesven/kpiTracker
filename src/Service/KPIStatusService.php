...
class KPIStatusService
{
    ...
    {
        return $this->getKpiStatus($kpi) === 'yellow';
    }
    ...
    {
        return $this->getKpiStatus($kpi) === 'red';
    }
    ...
    public function getKpisForReminder(array $kpis, int $daysBefore = 3, int $daysAfter = 0): array
    {
        ...
        foreach ($kpis as $kpi) {
            ...
            $hasValue = $this->kpiValueRepository->findByKpiAndPeriod($kpi, $currentPeriod) !== null;
    ...
    private function getNextQuarterStart(\DateTimeImmutable $date): \DateTimeImmutable
    {
        ...
        $year = $nextQuarterMonth === 1 ? (int) $date->format('Y') + 1 : (int) $date->format('Y');
